...
* The `data` folder contains the 42 CSV emoji files that we sourced from Kaggle. Each file has tweets that correspond to the emoji mentioned in the CSV's title.
* The `emoji_map.csv` file contains the emoji mapping data from the Amsterdam University of Applied Sciences. It has data on the description of each emoji as well as its ucode.
